...
Parser.prototype = {
  ...
  tag: function(tag){
    ...
    var loop = true;
    while (this.peek().type == "attrsplus") {
      var tok = this.advance();
        
      var obj = tok.attrs.attrs
      , escaped = tok.attrs.escaped
      , appended = tok.attrs.appended
      , names = Object.keys(obj);
      ...
      for (var i = 0, len = names.length; i < len; ++i) {
        ...
        if(tok.condition) {
          var modifier = "";
          if(tok.condition.operator == "unless") modifier = "!";
          val = "(" +modifier + "( " + tok.condition.expression +  ") ? (" + val + ") : null)"
        }
        tag.appendAttribute(name, val, escaped[name], appended[name]);
      }
      // newline*
      while ('newline' == this.peek().type) this.advance();
    }
