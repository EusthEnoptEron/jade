...
Lexer.prototype = {
  ...
  additionalAttrs: function() {
    var trigger = ">";
    var captures;
    if(this.input.charAt(0) == trigger && this.input.charAt(1) == " ") {
      this.consume(2);
      var attrs = this.attrs();
      if(attrs) {
        var tok = this.tok("attrsplus", "");
        tok.attrs = attrs;
        

        if(captures = this.input.match(/^[ ]*(if|unless)\b([^\n]+)/)) {
          this.consume(captures[0].length);
          tok.condition = {
            operator: captures[1],
          ...
          assertExpression(tok.condition.expression);
        }
        

        return tok;
      } else {
        this.input = trigger + " " + this.input;
      }
    }
  },
          ...
          switch (loc) {
            ...
            case 'key':
              if (key === '' && (str[i] === '"' || str[i] === "'")) {
                loc = 'key-char';
                quote = str[i];
              } else if (str[i] === '+' || str[i] === '!' || str[i] === equals) {
                appendedAttr = str[i] === '+';
                if (str[i] === '+') i++;
